...
export const ParkCard = ({ park, disabled, isEditMode, deleteItem, className }: ParkCardProps) => {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: park.id, disabled });
  ...
  return (
    ...
    >
      ...
      {isEditMode && deleteItem && (
        <Trash onClick={() => deleteItem(park)} className="absolute top-1 right-6" />
      )}
    </div>
  );
};
